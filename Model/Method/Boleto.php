...
class Boleto extends \Magento\Payment\Model\Method\AbstractMethod
{
    ...
    public function order(\Magento\Payment\Model\InfoInterface $payment, $amount)
    {
        ...
        try {
            ...
            $days_expire = $this->gatewayHelper->getBoletoDaysExpire();
            $instructions = $this->gatewayHelper->getBoletoInstructions();
            ...
            ->setCountry("BRA")
            ->setExpire(date('Y-m-d', strtotime("+$days_expire days")))
            ...
            if ($status === 'BOLETO' ||  $status === 0 ){
                $nrDocumento = '';
                if (isset($response->getResponse()['processor']['Boleto']['details']['nrDocument']))
                ...
                $payment->setAdditionalInformation('nrDocument', $nrDocumento);
                ...
                if (isset($response->getResponse()['processor']['Boleto']['details']['urlBoleto']))
                    $payment->setAdditionalInformation('boletoUrl', $response->getResponse()['processor']['Boleto']['details']['urlBoleto']);
                else if(isset($response->getResponse()['processor']['urlBoleto'])){
                    $payment->setAdditionalInformation('boletoUrl', $response->getResponse()['processor']['urlBoleto']);
                }  
            }else{
    ...
    private function getAcquirer(){
        $pm = $this->gatewayHelper->getBoletoAcquirer();
